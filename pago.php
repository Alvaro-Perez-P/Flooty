...
$nombre_usuario = $user_info["usuario"];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $reserva) {
    ...
    if (isset($_POST['accion']) && $_POST['accion'] === 'pagar') {
        ...
        if ($reserva["estado"] !== "aceptada") {
            $error = "Solo puedes pagar una reserva aceptada.";
        } else {
            ...
            if ($_conexion->affected_rows > 0) {
                ...
                $id_propietario = (int)$reserva["id_propietario"];
                $id_producto = (int)$reserva["id_producto"];
                $titulo_producto = $reserva["titulo"];
                $total_pagado = number_format((float)$reserva["total"], 2, ",", ".");

                $mensaje_notificacion_dueno = $_conexion->real_escape_string(
                    $nombre_usuario . " ha pagado " . $total_pagado . " € por la reserva del producto: " . $titulo_producto .
                    ". El cliente se pondrá en contacto para coordinar la entrega."
                );
                ...
                $_conexion->query("
                    ...
                    VALUES
                    ($id_propietario, $id_producto, $id_reserva, 'reserva_pagada', '$mensaje_notificacion_dueno')
                ");

                $mensaje_notificacion_cliente = $_conexion->real_escape_string(
                    "Tu pago de " . $total_pagado . " € por el producto " . $titulo_producto .
                    " fue registrado correctamente. Ahora puedes contactar al dueño desde Mis reservas."
                );
                ...
                $_conexion->query("
                    ...
                    (id_usuario, id_producto, id_reserva, tipo, mensaje)
                    VALUES
                    ($id_usuario, $id_producto, $id_reserva, 'pago_confirmado', '$mensaje_notificacion_cliente')
                ");
...
if ($reserva && !empty($reserva["imagenes"])) {
    $imgs = json_decode($reserva["imagenes"], true);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: Arial, Helvetica, sans-serif; }
        ...
        .alerta.ok {
            border: 1px solid rgba(151,183,112,0.4);
    ...
    <?php else: ?>
        ...
        <div class="grid">
            ...
            <div class="card-info">
                ...
                <?php if ($pagada && !$mensaje_pago): ?>
                    <div class="alerta ok">✅ Esta reserva ya está pagada.</div>
                <?php endif; ?>
                ...
                <div class="producto-mini">
                    ...
                    <div>
                        <div class="nombre"><?= htmlspecialchars($reserva["titulo"]) ?></div>

                        <div class="sub">
                            Reserva #<?= (int)$reserva["id"] ?> ·
                            <span class="badge-estado <?= $pagada ? 'badge-pagada' : '' ?>">
                ...
                <div class="fila-doble">
                    ...
                    <div>
                        ...
                        <div class="valor"><?= htmlspecialchars($reserva["estado"]) ?></div>
                    </div>
                </div>

                <a href="mis_reservas.php" class="btn-volver">
                    Volver a mis reservas
                </a>
            </div>

            <div class="card-info">
                <div style="font-size:17px;color:#97B770;font-weight:bold;margin-bottom:16px;">
                    Resumen del pago
                </div>
                ...
                <div class="resumen-fila">
                    <span>
                        ...
                        × <?= (int)$reserva["dias"] ?>
                        <?= ((int)$reserva["dias"] === 1) ? 'día' : 'días' ?>
                    </span>

                    <span>
                        <?= number_format(((float)$reserva["precio_dia"] * (int)$reserva["dias"]), 2, ",", ".") ?> €
                    </span>
                </div>
                ...
                <?php elseif ($reserva["estado"] === "pagada"): ?>
                    <div class="alerta ok" style="margin-top:14px;">
                        ✅ Pago registrado. Ya puedes contactar al dueño desde Mis reservas.
                    </div>
                <?php else: ?>
                    <div class="alerta" style="margin-top:14px;">
                        Esta reserva todavía no está aceptada para poder pagar.
                    </div>
                <?php endif; ?>
            </div>
