...
if (isset($_GET["favorito"])) {
    ...
    $consulta_favorito = "SELECT * FROM favoritos WHERE id_usuario = $id_usuario AND id_producto = $id_producto_favorito";
    $resultado_favorito = $_conexion->query($consulta_favorito);
    if ($resultado_favorito && $resultado_favorito->num_rows > 0) {
        $_conexion->query("DELETE FROM favoritos WHERE id_usuario = $id_usuario AND id_producto = $id_producto_favorito");
    } else {
        $_conexion->query("INSERT INTO favoritos (id_usuario, id_producto) VALUES ($id_usuario, $id_producto_favorito)");
    }
...
<head>
    ...
    <style>
        * {
        ...
        .profile-info, .menu-container, .categorias-container,
        .filtros-container, .productos-container, .publicidad-container, .footer-container {
        .profile-info,
        .menu-container,
        ...
        .profile-info, .menu-container, .categorias-container,
        .filtros-container, .productos-container { padding: 20px; }

        .img-avatar { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; margin-right: 10px; }
        .titulo-usuario { color: black; font-weight: bold; font-size: 28px; margin-bottom: 6px; display: flex; align-items: center; }
        .stars { font-size: 12px; color: #ffb400; margin-bottom: 6px; }
        .stars span { color: #888; }
        .profile-info small { font-size: 12px; color: #777; }

        .list-group-item {
            display: block; text-decoration: none; background: transparent; border: none;
            border-bottom: 1px solid rgba(0, 0, 0, 0.1); padding: 14px 12px;
            font-size: 16px; color: #333; transition: 0.2s;
        }
        .list-group-item:hover { background: rgba(151, 183, 112, 0.15); color: #97B770; }

        .btn { width: 100%; padding: 12px; font-size: 16px; font-weight: bold; border-radius: 6px; margin-top: 15px; }
        .btn-warning { background-color: #d8b24c; color: white; border: none; }
        .btn-warning:hover { background-color: #c49d35; color: white; }

        .titulo-bloque { font-size: 18px; color: #97B770; font-weight: bold; margin-bottom: 15px; }

        .categorias-select, .filtro-select {
            width: 100%; padding: 10px 12px; border-radius: 10px;
            border: 1px solid rgba(0, 0, 0, 0.15); background: #fff; color: #333; outline: none;
        }

        .filtros-lista { display: flex; flex-direction: column; gap: 12px; }
        .filtro-item { display: flex; align-items: center; gap: 10px; font-size: 14px; color: #333; }
        .filtro-item input[type="checkbox"] { accent-color: #97B770; }

        .productos-container { flex: 1; min-height: 500px; }
        .titulo-seccion { font-size: 24px; color: #97B770; font-weight: bold; margin-bottom: 20px; }

        .productos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }

        .tarjeta-producto {
            background: #fff; border-radius: 16px; border: 1px solid rgba(0, 0, 0, 0.08);
            overflow: hidden; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.08); transition: 0.3s; cursor: pointer;
        }
        .tarjeta-producto:hover { transform: translateY(-4px); }

        /* Contenedor de imagen: fondo beige para que el placeholder quede bien */
        .imagen-producto {
            width: 100%;
            height: 180px;
        ...
        .imagen-producto img { width: 100%; height: 100%; object-fit: cover; display: block; }

        /* Placeholder cuando la imagen no existe o falla al cargar */
        .img-placeholder {
        ...
        .contenido-producto { padding: 15px; }

        .categoria-producto {
            display: inline-block; font-size: 12px;
            background-color: rgba(151, 183, 112, 0.18); color: #6f8f4e;
            padding: 4px 10px; border-radius: 20px; margin-bottom: 10px;
        }

        .contenido-producto h3 { font-size: 18px; margin-bottom: 8px; color: #333; }
        .contenido-producto p { font-size: 14px; color: #666; margin-bottom: 8px; min-height: 42px; }
        .precio { font-size: 18px; font-weight: bold; color: #97B770; margin-bottom: 14px; }

        .acciones-producto { display: flex; justify-content: flex-end; align-items: center; }

        .btn-like {
            width: 42px; height: 42px; border: none; border-radius: 50%;
            background: rgba(245, 241, 230, 0.95); box-shadow: 0 3px 10px rgba(0, 0, 0, 0.12);
            display: flex; align-items: center; justify-content: center;
            cursor: pointer; font-size: 20px; transition: 0.2s; color: #666; text-decoration: none;
        }
        .btn-like:hover { transform: scale(1.08); }
        .btn-like.activo { background-color: #97B770; color: white; }

        .contenedor-boton-mas { display: flex; justify-content: center; margin-top: 25px; }
        .btn-mostrar-mas {
            min-width: 180px; padding: 12px 24px; border: none; border-radius: 10px;
            background-color: #97B770; color: white; font-size: 15px; font-weight: bold;
            cursor: pointer; transition: 0.3s; text-decoration: none; text-align: center;
        }
        .btn-mostrar-mas:hover { background-color: #7fa45a; color: white; }
        ...
        .publicidad-container {
            width: 100%; max-width: 250px; height: 500px;
        ...
        .footer-container { margin: 20px 10px 10px 10px; padding: 18px 25px; }
        ...
        @media (max-width: 992px) {
            ...
            .columna-izquierda, .productos-container, .publicidad-container { max-width: 100%; width: 100%; }
            .publicidad-container { min-height: 200px; }
...
<div class="contenedor-main">
    ...
    <div class="columna-izquierda">
        ...
        <?php if ($esta_logueado): ?>
            <div class="profile-info">
                <div class="titulo-usuario">
                    <img class="img-avatar" src="imagenes/logo-blanco.png" alt="avatar">
                    <?= htmlspecialchars($user_name) ?>
                </div>
                <div class="stars">★ ★ ★ ★ ★ <span>(35)</span></div>
        ...
        <div class="filtros-container">
            ...
            <div class="filtros-lista">
                <label class="filtro-item"><input type="checkbox"> Disponible hoy</label>
                <label class="filtro-item"><input type="checkbox"> Solo con envío</label>
                <label class="filtro-item"><input type="checkbox"> Mejor valorados</label>
            ...
            <?php if ($resultado_productos && $resultado_productos->num_rows > 0): ?>
                <?php while ($producto = $resultado_productos->fetch_assoc()): ?>
                    <?php
                        ...
                        $primera_imagen = (is_array($imagenes) && !empty($imagenes[0])) ? $imagenes[0] : "";
                        $es_favorito    = $esta_logueado && in_array((int)$producto["id"], $favoritos_usuario);
                    ?>
                    <div class="tarjeta-producto" onclick="window.location='producto.php?id=<?= $producto["id"] ?>'">
    ...
    <div class="publicidad-container">
        <div class="slide active"><img src="imagenes/banner1.png" alt="banner 1"></div>
        <div class="slide"><img src="imagenes/banner2.png" alt="banner 2"></div>
        <div class="slide"><img src="imagenes/banner3.png" alt="banner 3"></div>
    </div>
