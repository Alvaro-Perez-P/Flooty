...
/* ── Simulación de pago con Stripe (vía cURL) ── */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $reserva) {
    if (isset($_POST['accion']) && $_POST['accion'] === 'pagar') {
        ...
        $monto_total = (float)$reserva["total"];
        $monto_centavos = (int)round($monto_total * 100); // Stripe usa centavos
        $nombre_producto = $producto ? $producto["titulo"] : "Reserva #" . $reserva["id"];

        // Determinar URL base
        $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";
        $base_url = $protocol . "://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']);
        
        $success_url = $base_url . '/pago_exito.php?session_id={CHECKOUT_SESSION_ID}&id_reserva=' . $id_reserva;
        $cancel_url = $base_url . '/pago.php?id_reserva=' . $id_reserva . '&cancelado=1';
        ...
        $data = http_build_query([
            ...
            'line_items[0][price_data][product_data][name]' => $nombre_producto,
            'line_items[0][price_data][unit_amount]' => $monto_centavos,
        ...
        if ($response === false) {
            $error = "Error de conexión cURL: " . $curl_err;
            ...
            if (isset($response_data['url'])) {
                ...
                $error = "Error al crear la sesión de pago de Stripe.";
            }
        } else {
            $error_data = json_decode($response, true);
            if (isset($error_data['error']['message'])) {
                $error = "Error de Stripe: " . $error_data['error']['message'];
            } else {
                $error = "Error conectando con Stripe (HTTP $http_code). Verifica tu clave secreta.";
            }
        }
    }
}
    ...
    <?php else: ?>
        ...
        <div class="grid">
            ...
            <div class="card-info">
                ...
                <?php endif; ?>
                <?php if ($cancelado): ?>
                    <div class="alerta" style="border: 1px solid rgba(220,150,80,0.3); background: rgba(220,150,80,0.1); color: #b07030;">
                        ⚠️ Has cancelado el proceso de pago. Puedes volver a intentarlo cuando quieras.
                    </div>
                <?php endif; ?>
