...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($nombre == "") {
        ...
        $consulta_existe = "SELECT id FROM categorias WHERE nombre = '$nombre_seguro' LIMIT 1";
        $resultado_existe = $_conexion->query($consulta_existe);

        if ($resultado_existe && $resultado_existe->num_rows > 0) {
            $error = "Ya existe una categoría con ese nombre.";
        } else {
            $insert = "INSERT INTO categorias (nombre) VALUES ('$nombre_seguro')";

            if ($_conexion->query($insert)) {
                header("location: panel_categorias.php");
                exit();
            } else {
                $error = "Error al crear la categoría: " . $_conexion->error;
            }
        }
    }
}
...
<head>
<meta charset="utf-8">
<title>Crear categoría — Flooty</title>
